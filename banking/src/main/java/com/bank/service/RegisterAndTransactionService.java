...
public class RegisterAndTransactionService {
    ...
    public Long registerAccount(Register reg)  {
        ...
        transactionRepository.save( new TransactionHistory(accountNumber,null,money, TransactionType.DEPOSIT,money));
    ...
    public void transferMoney(MoneyTransfer m) {
        ...
        TransactionHistory receiver = new TransactionHistory(toAccount,fromAccount,receiver_last_txn.getClosingBalance()+moneyToTransfer,TransactionType.CREDIT,moneyToTransfer);
        transactionRepository.save(receiver);
        TransactionHistory sender = new TransactionHistory(fromAccount,toAccount,sender_last_txn.getClosingBalance()-moneyToTransfer,TransactionType.DEBIT,moneyToTransfer);
...
*/
 ...
 return historyList.stream().map(e->
   ...
   Double closingBalance = getTransactionHistory(fromAccount).get(0).getClosingBalance();
    ...
    @Transactional
    public void makeShoppingPayment(PaymentRequest p){
        ...
        TransactionHistory sender = new TransactionHistory(p.getFromAccountNo(),1234687678L,
                sender_last_txn.getClosingBalance()-p.getAmount(),TransactionType.DEBIT,p.getAmount());
        transactionRepository.save(sender);
